...
(function(){
  ...
  function getSignedUrl(token, filename, cb){
    var xhr = new XMLHttpRequest();
    var signUrl = SUPABASE_URL + '/storage/v1/object/sign/' + BUCKET + '/' + filename;
    console.log('[GeoBac] POST sign URL:', signUrl);
    ...
    xhr.timeout = 8000;
    xhr.onload = function(){
      console.log('[GeoBac] sign response status:', xhr.status);
      console.log('[GeoBac] sign response body:', xhr.responseText);
      try {
        var d = JSON.parse(xhr.responseText);
        if(d.signedURL){
          var fullUrl = SUPABASE_URL + d.signedURL;
          console.log('[GeoBac] final redirect URL:', fullUrl);
          cb(fullUrl);
        } else {
          console.log('[GeoBac] no signedURL in response, error:', d.error || d.message || 'unknown');
          cb(null);
        }
      } catch(e){ console.log('[GeoBac] parse error:', e); cb(null); }
    };
    xhr.onerror = function(){ console.log('[GeoBac] sign XHR error'); cb(null); };
    xhr.ontimeout = function(){ console.log('[GeoBac] sign XHR timeout'); cb(null); };
